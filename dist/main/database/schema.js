...
class DatabaseManager {
    constructor() {
        ...
        const dbPath = path_1.default.join(userDataPath, 'timetracker.db');
    ...
    initializeDefaultConfig() {
        ...
        const insert = this.db.prepare('INSERT OR IGNORE INTO Configuration (key, value) VALUES (?, ?)');
    ...
    setConfig(key, value) {
        this.db
            .prepare('INSERT OR REPLACE INTO Configuration (key, value, updated_at) VALUES (?, ?, CURRENT_TIMESTAMP)')
            .run(key, value);
    }
    // WorkSession methods
    createWorkSession(session) {
        const result = this.db
            .prepare(`INSERT INTO WorkSession (issue_key, issue_title, issue_type, start_time, end_time,
         duration_seconds, comment, status, tempo_worklog_id)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`)
            .run(session.issue_key, session.issue_title, session.issue_type, session.start_time, session.end_time, session.duration_seconds, session.comment, session.status, session.tempo_worklog_id);
        return result.lastInsertRowid;
    }
    updateWorkSession(id, updates) {
    ...
    getWorkSessionsByDateRange(startDate, endDate) {
        return this.db
            .prepare('SELECT * FROM WorkSession WHERE date(start_time) BETWEEN ? AND ? ORDER BY start_time')
            .all(startDate, endDate);
    }
    // DailySummary methods
    createOrUpdateDailySummary(summary) {
        this.db
            .prepare(`INSERT INTO DailySummary (date, total_minutes, adjusted_minutes, status, sent_at)
         VALUES (?, ?, ?, ?, ?)
         ...
         sent_at = excluded.sent_at`)
            .run(summary.date, summary.total_minutes, summary.adjusted_minutes, summary.status, summary.sent_at);
    }
    ...
    getDailySummariesByDateRange(startDate, endDate) {
        return this.db
    ...
    addRecentIssue(issue) {
        this.db
            .prepare(`INSERT INTO RecentIssue (issue_key, issue_title, issue_type, epic_key, last_used_at)
    ...
    addFavoriteTask(task) {
        ...
        this.db
            .prepare(`INSERT INTO FavoriteTask (issue_key, issue_title, issue_type, position)
         VALUES (?, ?, ?, ?)
         ON CONFLICT(issue_key) DO UPDATE SET
         issue_title = excluded.issue_title,
         issue_type = excluded.issue_type,
    ...
    addKeyboardShortcut(shortcut) {
        this.db
            .prepare(`INSERT INTO KeyboardShortcut (accelerator, issue_key, issue_title, issue_type, enabled)
         VALUES (?, ?, ?, ?, ?)
    ...
    removeKeyboardShortcut(accelerator) {
        this.db
    ...
    updateKeyboardShortcut(accelerator, updates) {
        const fields = [];
        const values = [];
